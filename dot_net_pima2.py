...
try:
  for k in number_list:
    ...
    encodedFields = urllib.urlencode({'statecode':k, 'taxyear':2012, 'date':'08/19/2016','submit': "SEARCH"})
    ...
    output["Tax Year"] = 2012
    total_due = su.find("th", text=re.compile(r'TOTAL DUE:'))
    if total_due is not None:
      total_due = su.find("th", text=re.compile(r'TOTAL DUE:')).findNext('td').text
      print total_due
    output["FY 2012 DUE"] = total_due
...
#   csv_columns =['ParcelNumber', 'Tax Year','FY 2015 DUE']
